...
class UserWorkspaceRoleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if self.instance:
            if (
                UserWorkspaceRole.objects.filter(
                    user=data.get("user", self.instance.user), workspace=data.get("workspace", self.instance.workspace)
                )
            ...
            if UserWorkspaceRole.objects.filter(user=data["user"], workspace=data["workspace"]).exists():
                raise serializers.ValidationError("This user already has a role in this workspace.")
        return data
